...
def select_harness_backend(requested: str | None = None) -> str:
    """Resolve the harness backend (TL-1).

    An explicit request wins. Otherwise prefer cocotb when it is importable
    and fall back to the cocotb-free verilator-native path — a Verilator-only
    machine should never see "install cocotb" for a run the native backend
    handles identically (TL-2).
    """
    if requested in HARNESS_BACKENDS:
        return requested
    import importlib.util

    return "cocotb" if importlib.util.find_spec("cocotb") is not None else "verilator"
...
def compare_streams(
    ...
    fstate: dict[str, list[float]] = {}  # z^-1 history threads across the stream (SD-1)
    for vec in vectors:
        ref = evaluate_float(audit.dag, dict(vec.items()), fmt, state=fstate)
...
def run_cosim(
    ...
    has_delay = any(audit.dag.nodes[nid].module == "delay" for nid in audit.dag.order)
    if has_delay and cadence != "continuous":
        # a z^-1 register shifts every clock; under bubbles it would capture
        # garbage between valid samples — the golden model cannot match (SD-1)
        raise ValueError(
            "designs with delay() state require the continuous cadence "
            "(the nkMatlib contract: one sample per clock)"
        )
    if vectors is None:
    ...
    # cocotb 2.x: trace support is requested via build(waves=True) — a raw
    # --trace build_arg is invisible to the runner's own waves check and it
    # aborts with "--trace requires the design to be built with trace support"
    waves_kwarg = ", waves=True" if trace else ""
...
runner.build(
    ...
    build_args=["-Wno-fatal"]{waves_kwarg},
)
runner.test(hdl_toplevel="tb_wrapper", test_module="tb_cosim"{waves_kwarg})
""",
        encoding="utf-8",
    )
    env = dict(os.environ)
