...
def render_query_script(script: Path | None, setup: Path | None, out_json: Path) -> str:
    ...
    With ``script=None`` only the setup is loaded/run — inspecting a `.mat`
    parameter file by itself.
    """
    setup_lit = _mq(setup) if setup is not None else ""
    script_lit = _mq(script) if script is not None else ""
