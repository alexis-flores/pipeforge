...
class MatlabConfig:
    ...
    warm: bool = False  # keep a background MATLAB session alive
    auto_refresh: bool = False  # re-snapshot when watched files change (needs warm)
    ...
    def load(cls) -> MatlabConfig:
        ...
        cfg.warm = bool(data.get("matlabWarm", False))
        cfg.auto_refresh = bool(data.get("matlabAutoRefresh", False))
        return cfg
    ...
    def save(self) -> None:
        ...
        data["matlabWarm"] = self.warm
        data["matlabAutoRefresh"] = self.auto_refresh
...
def render_snapshot_function() -> str:
    """The pf_snapshot function MATLAB runs to dump a workspace as JSON.

    Parameterized (script, setup, out) so the same file serves both the
    one-shot ``-batch`` mode and the persistent session server. Locals are
    pf_q_-prefixed (and args pf_q_a-prefixed) and filtered out; the user
    script runs inside the function workspace so nothing leaks between
    snapshots. Script errors land in the JSON 'error' field instead of
    losing the whole snapshot. An empty script loads only the setup —
    inspecting a `.mat` parameter file by itself.
    """
    return f"""function pf_snapshot(pf_q_ascript, pf_q_asetup, pf_q_aout)
% generated by PipeForge — workspace snapshot query. Do not edit.
pf_q_out = pf_q_aout;
pf_q_setup = pf_q_asetup;
pf_q_script = pf_q_ascript;
pf_q_doc = struct();
...
def _cache_key(script: Path | None, setup: Path | None, command: list[str]) -> str:
    h = hashlib.sha256()
...
def take_snapshot(
    ...
    ``script=None`` loads/runs only the setup — inspecting a `.mat`
    parameter file (or a setup script) by itself. Cached by
    (script, setup, mtimes, command); ``force=True`` is the explicit
    refresh. Raises MatlabUnavailable with an actionable message on any
    failure (C2).
    """
    cfg = config if config is not None else MatlabConfig.load()
    if setup is None:
        setup = cfg.setup
    if script is None and setup is None:
        raise MatlabUnavailable(
            "nothing to snapshot: provide a script, a setup file (.m or .mat), "
            "or configure one in Settings"
        )
    ...
    (work / "pf_snapshot.m").write_text(render_snapshot_function(), encoding="utf-8")
    script_arg = _mq(script) if script is not None else ""
    setup_arg = _mq(setup) if setup is not None else ""
    cmd = [
        *cfg.command,
        "-batch",
        f"cd('{_mq(work)}'); pf_snapshot('{script_arg}', '{setup_arg}', '{_mq(out_json)}')",
    ]
    if log:
        log("matlab: " + " ".join(cmd))
    try:
...
def snapshot_target(
    ...
    """Snapshot whatever the user points at: a `.m` script or a `.mat` alone.

    A `.mat` target is loaded by itself (no script run) — browsing a
    parameter file's variables and types. Combining a `.mat` target with a
    separate setup is ambiguous and rejected.
    """
    if path.suffix.lower() == ".mat":
        if setup is not None:
            raise MatlabUnavailable(
                f"'{path.name}' is already a data file; --setup does not apply. "
                "Snapshot the .mat alone, or snapshot a .m script with this "
                "file as its setup."
            )
        return take_snapshot(None, setup=path, config=config, force=force, log=log)
    return take_snapshot(path, setup=setup, config=config, force=force, log=log)
...
def snapshot_auto(
    ...
    """Snapshot via the warm session when one is alive, else batch mode.

    The warm path is always fresh (it is fast enough that caching would only
    hide changes); a dead/slow session logs and falls back to batch in the
    same call, so callers never have to care which path ran.
    """
    from pipeforge.services.matlab_session import request_snapshot, server_alive
    ...
    if server_alive():
        if log:
            log("matlab: using the warm session")
        try:
            return request_snapshot(
                script.resolve() if script else None,
                setup.resolve() if setup else None,
            )
        except MatlabUnavailable as exc:
            if log:
                log(f"matlab: {exc}")
    return take_snapshot(script, setup=setup, config=cfg, force=force, log=log)
